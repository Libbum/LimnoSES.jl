...
function dNutr(m::ABM{Household})
    ...
    #(model.properties[:recycling_rate]*model.properties[:lake].nutrients+sewage_water(model)) - (model.properties[:recycling_rate]*model.properties[:lake].nutrients)
    sewage_water(m)
end
...
function update_settings!(m::ABM{Household})
    if m.identifier != "none"
        ...
        elseif m.year > 11
            ...
            if (m.nutrient_series == TransientUp() && round(m.target_nutrients; sigdigits=2) >= m.target_nutrients) || (m.nutrient_series == TransientDown() && round(m.target_nutrients; sigdigits=2) <= m.target_nutrients)
                if m.identifier == "biggs-baseline" && m.nutrient_series == TransientUp()
                    ...
                    #time-to-finish = 31
                    m.transient_nutrients = m.target_nutrients
                end
...
function update_nutrients!(m::ABM{Household})
    if m.nutrient_series == TransientUp()
        temp = m.transient_nutrients + m.nutrient_change
        m.transient_nutrients = temp > m.target_nutrients ? m.target_nutrients : temp
